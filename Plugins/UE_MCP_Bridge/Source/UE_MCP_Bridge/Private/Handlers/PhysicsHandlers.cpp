...
TSharedPtr<FJsonValue> FPhysicsHandlers::SetCollision(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Params->TryGetObjectField(TEXT("responses"), ResponsesObj) && ResponsesObj && (*ResponsesObj).IsValid())
	{
		for (const auto& Pair : (*ResponsesObj)->Values)
		{
			...
			if (!ResolveCollisionChannel(KeyStr, Ch))
			{
				return MCPError(FString::Printf(TEXT("Unknown response channel '%s'"), *KeyStr));
			}
			FString RespStr;
			...
			{
				return MCPError(FString::Printf(TEXT("Unknown response '%s' for channel '%s'. Use Block, Overlap, Ignore."), *RespStr, *KeyStr));
			}
