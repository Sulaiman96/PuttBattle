...
## 11. Editor MCP tools (native unreal-mcp)
One MCP server, **`unreal-mcp`**, is configured in `.mcp.json` (setup: `docs/MCP-SETUP.md`): Epic's **native** in-editor Model Context Protocol server (HTTP at `http://127.0.0.1:8000/mcp`, started by the `ModelContextProtocol` plugin — Editor Prefs → Model Context Protocol → Auto Start Server). It is a **toolset registry**: three discovery meta-tools dispatch everything — `list_toolsets`, `describe_toolset(name)`, and `call_tool(tool_name, toolset_name, arguments)`. Breadth comes from the enabled `*Toolset` plugins (`EditorToolset`, `UMGToolSet`, `NiagaraToolsets`), exposing SceneTools / ActorTools / ObjectTools / PrimitiveTools / AssetTools / BlueprintTools / MaterialTools / a `ProgrammaticToolset.execute_tool_script` batcher, and more. The third-party `ue-mcp` node client + `UE_MCP_Bridge` plugin were **removed (D-17)**; this native server is the sole editor MCP. The editor must be open; connect by (re)starting the Claude session so it reads `.mcp.json`.

**Key limitation:** `ObjectTools.set_properties` writes a mesh component's `staticMesh` / transform / `physMaterialOverride` / `overrideMaterials`, but **cannot** change a component's collision channel/enabled (`PB_Floor`/`PB_Wall` `objectType`, `collisionEnabled`) — those need `FBodyInstance` setters. Author/finish PB_Floor/PB_Wall collision via the editor Details panel, or by re-shaping an already-channeled mesh (its channel carries over untouched). See `docs/LEARNING.md`.

Rules (non-negotiable):
1. **Editor-touching task? Check first.** Confirm the server responds at session start (e.g. `SceneTools.get_current_level`). If the editor is closed or the server is down, do NOT stall or improvise: complete the C++/data/asset-definition work, then list the editor-side steps for the human under a "Requires editor" heading.
2. **Discover before you assume.** Tools are dispatched via `call_tool` (a `toolset_name` + a bare `tool_name`); call `describe_toolset` for the exact input schema before invoking, and read state back (get before set). Never guess tool names or required params.
3. **Full asset paths** (`/Game/Data/Powerups/DA_Gust`), never bare names. **Compile a Blueprint (`BlueprintTools.compile_blueprint`) before adding variable/graph nodes that reference its members.**
4. **One asset-lifecycle path:** use `AssetTools` for asset lifecycle (find/load/save/duplicate/move/delete; `save_assets(["…"])` saves a level) — never ad-hoc; `move` for renames, never duplicate+delete.
...
6. **Verify visually**: after scene/Blueprint changes, `EditorAppToolset.CaptureViewport` (or read logs) and confirm the result rather than assuming success.
...
8. The tool allow-list and any auto-approve are **human decisions** — never edit `.claude/settings.json` permissions as part of a task. (`GameplayAbilities` was only ever enabled as the old bridge's hard dependency and is **removed** with it — never author or enable GAS, D22.)
9. For UE 5.8 API questions, prefer the engine source in `Engine/Source` (§10) over guessing.
10. **Lay out graph nodes readably — never stack them.** When creating or editing any Blueprint/widget graph, give every node its own clear position: flow left→right along exec wires, keep generous horizontal spacing between columns and vertical spacing between parallel branches, and never drop a new node on top of (or overlapping) an existing one. Two nodes sharing the same `posX/posY` is a bug — a graph the human opens must be legible without untangling. After authoring nodes, set positions explicitly via the `BlueprintTools` graph/node operations and confirm no overlaps.
